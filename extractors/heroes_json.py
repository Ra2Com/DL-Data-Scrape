...
def create_heroes_json():
    ...
    for hero in heroes_vdata["Root"]:
        ...
        heroes.append({
            ...
            "role": extract_localization_ability(hero + "_role"),
            "playstyle": extract_localization_ability(hero + "_playstyle"),
            "lore": extract_localization_ability(hero + "_lore"),
            "image": hero_image,
            "abilities": hero_abilities
        })

    Path("data/heroes.json").write_text(json.dumps(heroes, ensure_ascii=False, indent=4), encoding="utf-8")
